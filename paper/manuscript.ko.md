...
## 초록
이미지 편집 시스템에서 앞선 생성물을 다음 입력으로 사용하면 요청하지 않은 얼굴 변화가 누적될 수 있다. 본 연구는 순차 편집과 최초 원본에 누적 요구사항을 적용하는 재생성을 비교한다. 초기 실험은 합성 인물 6명과 순차 7경로, 후속 통제 실험은 기존 3인물의 98개 출력을 포함한다. 얼굴 MAE·SSIM·LPIPS, 피부 내부 영역 및 별도 AI 등급으로 분석하였다. 세 인물의 후속 비교에서 일괄 편집의 평균 얼굴 LPIPS가 순차보다 낮았다. 별도 FLUX.2 Klein 4B 실험의 두 시드 평균 LPIPS는 일괄 0.091313, 순차 0.124455였으나 피부 원시 MAE는 일부 설정에서 반대 방향이었다. 최초 원본과 누적 요구를 분리하는 웹 편집기를 구현하였다. 웹 생성 경로를 합성 인물 6명·3시드로 확장한 18쌍에서 평균 얼굴 LPIPS는 일괄 0.087120, 순차 0.108710이었으며, 일괄 방식이 18쌍 모두 낮았다.
...
본 연구는 직전 출력과 최초 원본을 사용하는 두 입력 정책을 비교하고, 얼굴 전체 거리와 피부 내부 지표가 다른 방향을 보이는 사례를 분석하며, 누적 요청을 관리하는 웹 편집기에 두 방식을 구현하였다.
...
웹 편집기 비교에는 합성 인물 6명과 인물당 세 번의 생성 반복을 사용했다. 후속 통제 비교에는 그중 3명이 포함되었다. 동일 인물의 반복과 정책 간 공유 출력은 종속된 관측값이다. 초기 생성기의 백엔드 버전과 시드는 확보되지 않았고, AI 등급은 동일 기반 모델의 편향을 공유할 수 있다. ROI 민감도 분석은 결과를 확인한 뒤 설계했다.
