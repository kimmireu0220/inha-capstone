...
## 부록 A. 자료 및 재현 범위
분석 기준은 저장소의 CURRENT_STATUS.md와 개별 실험 결과 기록이다. 초기 정책 결과는 experiments/nonhuman-followup-v1/, 후속 통제는 experiments/autonomous-nonhuman-v1/ 및 experiments/edit-decomposition-v1/, 후속 AI 평가는 experiments/followup-ai-v1/, 로컬 비교는 experiments/local-policy-v1/, 추가 민감도 분석은 experiments/local-policy-robustness-v1/에 있다. 본문 표와 원자료의 연결은 paper/evidence.json에 기록한다.

공개 가능한 코드·요약·지표·합성 이미지는 저장소에 보존한다. 자격 증명, 로컬 작업 데이터 및 모델 가중치는 포함하지 않는다. 저장된 이미지의 수치 분석은 해당 의존성과 입력을 갖춘 환경에서 재실행할 수 있다. 사이트 테스트는 기능 검증이며 사용자 대상 평가가 아니다.
