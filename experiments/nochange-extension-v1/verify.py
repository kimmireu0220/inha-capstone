...
def main():
    ...
    assert {j['person'] for j in jobs.values()} == {'P04'}
    assert {j['order'] for j in jobs.values()} == {'no_change'}
    assert len({j['prompt'] for j in jobs.values() if j['stage']==6}) == 1
    for person in ('P04',):
        for arm in ('no_change',):
            assert result['summary'][person+':final:'+arm]['n_paths'] == 2
    assert {(j['repeat'],j['stage']) for j in jobs.values()} == {(r,s) for r in (1,2) for s in (4,5,6)}
    ...
    save(ROOT/'verification.json', {'passed': True, 'complete_outputs': 6,
                                    'independent_fixed_metric_pairs': 12, 'max_abs_error': error,
