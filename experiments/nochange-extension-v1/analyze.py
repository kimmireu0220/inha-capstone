...
def main():
    ...
    assert len(plan['jobs']) == 6 and len({j['id'] for j in plan['jobs']}) == 6
    jobs = {j['id']: j for j in plan['jobs'] + plan['parents']}
    for path, digest in plan['prior_files_sha256'].items():
        assert M.sha(local(path)) == digest
    completed = [j for j in plan['jobs'] if local(j['output']).exists()]
    if not args.partial:
        assert len(completed) == 6, len(completed)
    ...
    result = {'complete': len(rows) == 6, 'planned': 6, 'measured': len(rows), 'rows': rows,
