...
.venv-metrics/bin/python experiments/ten-stage-p03-v1/interventions-stage4/metrics/compute.py
...
def main():
    ...
    assert all(j['stage'] == 4 and all('- ' + requirement in j['prompt'] for requirement in REQUIREMENTS) for j in jobs)
    assert all(list(j['state'].values()) == REQUIREMENTS for j in jobs)
    sources = {(r, 'baseline'): EXP / f'run-{r}/step-4.png' for r in [1, 2]}
    for job in jobs:
    ...
    with torch.no_grad():
        ...
        for run in [1, 2]:
            for method in METHODS:
                ...
                row = {'run': run, 'stage': 4, 'method': method, 'source_sha256': sha(sources[(run, method)]),
                ...
                if method == 'baseline':
                    old = next(r for r in prior['results'] if r['run'] == run and r['step'] == 4)
    ...
    save_json(OUT / 'provenance.json', {
        ...
        'sequential_metrics_sha256': sha(prior_path), 'protocol_sha256': sha(INTERVENTIONS / 'PROTOCOL.md'),
