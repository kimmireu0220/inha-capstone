...
def main():
    ...
    assert len(plan['jobs']) == 24 and len({j['id'] for j in plan['jobs']}) == 24
    jobs = {j['id']: j for j in plan['jobs']}
    completed = [j for j in plan['jobs'] if local(j['output']).exists()]
    if not args.partial:
        assert len(completed) == 24, len(completed)
    ...
    result = {'complete': len(rows) == 24, 'planned': 24, 'measured': len(rows), 'rows': rows,
