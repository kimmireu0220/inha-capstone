...
def main():
    ...
    assert result['complete'] and len(result['rows']) == 24
    jobs = {j['id']: j for j in plan['jobs']}
    assert set(jobs) == {r['id'] for r in result['rows']}
    assert {j['person'] for j in jobs.values()} == {'P04', 'P05', 'P06'}
    assert {j['order'] for j in jobs.values()} == {'gray_original', 'navy_original', 'gray_cable', 'navy_cable'}
    assert all(j['stage'] == 1 for j in jobs.values())
    for person in ('P04', 'P05', 'P06'):
        for arm in ('gray_original', 'navy_original', 'gray_cable', 'navy_cable'):
            ...
            assert len({j['prompt'] for j in jobs.values() if j['person']==person and j['order']==arm}) == 1
    ...
    save(ROOT/'verification.json', {'passed': True, 'complete_outputs': 24,
                                    'independent_fixed_metric_pairs': 48, 'max_abs_error': error,
                                    'csv_rows_verified': len(flat), 'within_condition_repeat_prompt_identical': True,
