...
def main():
    ...
    assert M.sha(local(plan['reference'])) == plan['reference_sha256']
    assert len(plan['jobs']) == 38 and len({j['id'] for j in plan['jobs']}) == 38
    jobs = {j['id']: j for j in plan['jobs']}
    completed = [j for j in plan['jobs'] if local(j['output']).exists()]
    if not args.partial:
        assert len(completed) == 38, len(completed)
    ...
    ref = M.image(local(plan['reference']))
    control = M.measure(ref, ref, plan['roi'], network, True)
    for mode in M.MODES:
    ...
    for job in completed:
        ...
        if old and old['signature'] == key:
            ...
            a = M.image(path)
            original = M.measure(a, ref, plan['roi'], network, True)
            previous = original if job['stage'] == 1 else M.measure(a, M.image(inp), plan['roi'], network, True)
            row = {k: job[k] for k in ('id', 'order', 'repeat', 'stage', 'added', 'completed_set')}
    ...
    for r in rows:
        if r['stage'] == (1 if r['order'] == 'batch' else 3):
            groups['final:' + r['order']].append(r)
        if r['order'] != 'batch':
            groups[f"stage:{r['stage']}:added:{r['added']}"].append(r)
            if r['stage'] == 2:
                groups['two_step:' + r['order'][:2]].append(r)
    summary = {}
    ...
    result = {'complete': len(rows) == 38, 'planned': 38, 'measured': len(rows), 'rows': rows,
    ...
    save(ROOT / 'manifest.json', {'signature': signature, 'files_sha256': hashes, 'roi': plan['roi'],
                                 'skin_regions': M.REGIONS, 'device': 'cpu', 'threads': 2,
    ...
    with (ROOT / 'metrics.csv').open('w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['id', 'order', 'repeat', 'stage', 'added', 'completed_set',
        ...
        for r in rows:
            for basis in ('original', 'previous'):
                for mode in M.MODES:
                    base = {**{k: r[k] for k in ('id', 'order', 'repeat', 'stage', 'added', 'completed_set')},
