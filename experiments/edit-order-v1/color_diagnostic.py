"""Post-hoc numerical decomposition; never writes modified image files."""
import json
...
def main():
    ...
    for row in result['rows']:
        ...
        out = {k: row[k] for k in ('id', 'order', 'repeat', 'stage', 'added')}
    ...
    save(ROOT/'color-results.json', {'status': 'post_hoc_exploratory', 'rows': rows, 'summary': groups,
                                     'controls': {'identity': identity, 'constant_offset': offset},
                                     'source_results_sha256': M.sha(ROOT/'results.json'),
                                     'plan_sha256': M.sha(ROOT/'POSTHOC_COLOR.md'),
                                     'analysis_sha256': M.sha(Path(__file__)), 'checks_passed': True})
    print(json.dumps({k: v['original']['fixed'] for k, v in groups.items() if k.startswith('stage:1')}, ensure_ascii=False))
