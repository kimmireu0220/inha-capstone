...
export default function Survey() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (
          ...
          setIndex(next < 0 ? candidates.length * 2 : next);
        }
      }
    } catch {
  ...
  async function submit() {
    ...
    setError('');
    try {
      const r = await fetch('/api/followup/submit', {
        method: 'POST',
  ...
  if (!draft)
    return (
      <main className="trigger-review">
        <section className="survey-intro">
          <span className="eyebrow">인물 이미지 평가 · 총 24문항</span>
          <h1>
            편집된 얼굴을
            <br />
            어떻게 보셨나요?
          ...
          <p>
            원본과 편집된 이미지를 비교해 피부의 부자연스러운 변화와 인물 얼굴
            보존 정도를 평가해주세요. 정답은 없습니다.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
