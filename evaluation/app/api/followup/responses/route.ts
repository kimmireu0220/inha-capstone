...
import { SCHEMA, tasks, type Submission } from '@/lib/followup-survey';
type Row = {
  id: string;
  received_at: string;
  age: string;
    ...
    if (download) {
            ...
            for (;;) {
              ...
              for (const row of page.results) {
                const d = JSON.parse(row.payload) as Submission & {
                  ...
                  displayRegions: Record<string, number[]>;
                };
                  ...
                  controller.enqueue(
                    encoder.encode(
                      [
                        ...
                        t.kind === 'face' ? JSON.stringify(d.displayRegions[t.candidate.id]) : 'full',
                      ]
                        .map(cell)
                        .join(',') + '\r\n',
                    ),
      ...
      return new Response(stream, {
        headers: {
          ...
          'Content-Disposition': 'attachment; filename="followup-human-evaluation.csv"',
