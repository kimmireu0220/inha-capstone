...
import { validSubmission, tasks, candidates, type Submission } from '@/lib/followup-survey';
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!validSubmission(input))
      return reply(
        { error: '참여 정보와 24개 응답을 모두 확인해주세요.' },
        400,
      );
    const d = {
      schema: input.schema,
    ...
    const payload = JSON.stringify({ ...d, imageHashes, displayRegions: Object.fromEntries(candidates.map(c => [c.id, c.roi])), imageHashBasis: "full_png_with_css_roi" });
